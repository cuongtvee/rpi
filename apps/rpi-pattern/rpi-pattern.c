...
int main(int argc, char *argv[])
{
    ...
    printf("Raspberry Pi Camera Mode\n");
    if(argc < 2){
    ...
    if(i2c_reg_read(i2c_file, CAMERA_IIC_ADDRESS, 0x503D)){
        printf("Switch to camera mode.\n");
        i2c_reg_write(i2c_file, CAMERA_IIC_ADDRESS, 0x503D, 0x00);
    }
    else{
        printf("Switch to pattern mode.\n");
        i2c_reg_write(i2c_file, CAMERA_IIC_ADDRESS, 0x503D, 0x80);
    }
    close(i2c_file);
	return 0;
}
